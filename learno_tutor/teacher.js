...
var wiziqUrl = 'http://wiziq.authordm.com';
		...
		function (error, response, body){
			...
			if(!error && response.statusCode == 200){
				...
				if(bodyObj.wiziqId == 0){					
					...
					if(bodyObj.AcademySubdomainName == ""){
						//this is a learner, throw him out!
						return res.json({success: false, message: "You seems to be an existing LEARNER. Please try Signup instead.", statusCode: 200});
					}
...
function linkTeacherInfoWithTeacher(req, res, next, teacher, tchrInfo){
	...
	TeacherInfo.findByIdAndUpdate(tchrInfo._id, dataToUpdate, function(err, post){
		...
		//returnTeacher(req, res, next,teacher_id);
		return res.json({success:true, teacher: teacher});	// res.json(teacher);
	});
}

function returnTeacher(req, res, next, teacher_id){
	console.log('returnTeacher()');
	Teacher.findOne({_id: teacher_id}, function(err, tea){
		if(err) return next(err);
		if(tea) return res.json({success:true, teacher: tea, statusCode: 200});
	});
}
